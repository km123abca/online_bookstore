 ...
 if(isset($_GET['id']))
 	{
 		...
 		if(empty($id))
 			...
 			{ 	
 				...
 				// store2debug("SELECT * FROM books WHERE category_id=".$id);
 				...
 				if($stmt2->rowCount() > 0)
					...
					foreach ($books as $key => $book) 
						{
						$em=$em.$book['title'].", ";
						$em=substr($em, 0,strlen($em)-2)." uses this category, first delete those titles or change category in those";
						header("Location:../admin.php?error=$em");
		 				exit;
						}
 					}
					...
					if($res)
	 					{		 					
		  				$sm="Category successfully deleted";
